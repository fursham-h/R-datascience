...
> 3) Interconvert between absolute and relative paths.
> 4) View, search within, copy, move, and rename files. Create new directories.
> 5) Use wildcards (*) to perform operations on multiple files.
> 6) Employ the `grep` command to search for information within files.
> 7) Print the results of a command to a file.
> 8) Explain how a FASTQ file encodes per-bClean FASTQ reads using Trimmomatic.ase quality scores.
> 9) Interpret a FastQC plot summarizing per-base quality across all reads.
> 10) Clean FASTQ reads using Trimmomatic.
> 11) Align and quantify reads using Kallisto
><br />
